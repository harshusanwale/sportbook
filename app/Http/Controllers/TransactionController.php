...
class TransactionController extends Controller
{
    ...
    {
        $players = Player::all();
        $tranlist = Transaction::with('agent')->get();
    // Retrieve player names using player IDs
        $tranlist->each(function ($transaction) {
        $playerName = Player::where('id', $transaction->player_name)->value('username');
        $transaction->player_name = $playerName;
        });
        // Return the view with the transaction list
    ...
    {
        $transdata = Transaction::where('id',$id)->with('agent','period','bet_type')->first();
        $player_name = Player::where('id',$transdata->player_name)->first();
        $transdata->player_name = $player_name->username;
